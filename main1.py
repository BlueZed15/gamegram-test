...
async def save_json(level_file: UploadFile = File(...),sandbox_id=Annotated[str,Form(...)],creator_id=Annotated[str,Form(...)]):
    global json_data
    json_data["game_id"]=uuid.uuid4()
...
@router.get("/getjson")
def get_json(game_id=Annotated[str,Form(...)]):
